...
scenario_00 = ScenarioConfig(
    ...
    agents=[
        AgentConfig(
            ...
            scheduled_tasks=[
                TaskInstance(schema=deliver_item, 
                             bindings={Var("?item"): Const("item_3")}),
                TaskInstance(schema=deliver_item, 
                             bindings={Var("?item"): Const("item_2")}),
            ],
            observes=[],
        ),
        AgentConfig(
            ...
            scheduled_tasks=[
                TaskInstance(schema=deliver_item, 
                             bindings={Var("?item"): Const("item_4")}),
                TaskInstance(schema=deliver_item, 
                             bindings={Var("?item"): Const("item_6")}),
                TaskInstance(schema=deliver_item, 
                             bindings={Var("?item"): Const("item_7")}),
            ],
            observes=["human_0"],
        ),
    ],
