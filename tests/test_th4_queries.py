...
task equality (same_task). s02 abandon, s11 interruption and resumption, s85 /
s92 binding level, s70 the empty stack, s04 human-only tasks; a task model
...
def test_s02_abandon():
    m = model_for("env_layout0", "scenario_02")
    record = run_script(m)
    cfg = human_cfg("env_layout0", "scenario_02")
...
def test_s11_interruption_and_resumption():
    ...
    record = run_script(m)
    cfg = human_cfg("env_layout1", "scenario_11")
...
@pytest.mark.parametrize("layout,sid,item", [("env_layout8", "scenario_85", "item_0"),
                                              ("env_layout9", "scenario_92", "item_2")])
...
def test_s70_empty_stack_after_the_script():
    m = model_for("env_layout7", "scenario_70")
    for _ in range(300):
        m.step()
    record = m.humans[H].record
    cfg = human_cfg("env_layout7", "scenario_70")
...
def test_task_absent():
    m = model_for("env_layout0", "scenario_04")
    robot = m.observing[R]
    door, st, corner, delivery = script_tasks(human_cfg("env_layout0", "scenario_04"))
    ...
    # a PersonalTask omitted from the task model (env_layout1 has the coffee machine)
    m = model_for("env_layout1", "scenario_11")
    robot = m.observing[R]
...
def test_enumerated_binding_absent_carries_the_first_in_parameter_order():
    m = model_for("env_layout8", "scenario_85")
    robot = m.observing[R]
...
@pytest.mark.parametrize("layout,sid", [("env_layout1", "scenario_11"), ("env_layout8", "scenario_85")])
