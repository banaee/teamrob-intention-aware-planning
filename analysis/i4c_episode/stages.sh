#!/usr/bin/env bash
# stages.sh — regenerate the I4c replay. Logs are gitignored (*.log); this is how they come back.
#   baseline : I4b code (d040e6d), run_mesa.py                                  -> baseline/
#   new      : HEAD (I4c: empty stretch = no factor, episode boundary), run_mesa.py -> new/
#   harness  : check_i4c.py --final: the instrumented base (cmp'd against new/), the variants empty_only,
#              episode_only and neither (cmp'd against baseline/); summary.md, metrics.csv, the CSVs,
#              retention.csv, chains.md, region.md
#   diffs    : diff_ir.py (I2's) baseline -> new, and new -> each variant, per condition
# Usage: analysis/i4c_episode/stages.sh   (from the repo root; needs git worktree, ~8 min)
set -e
HERE=$(cd "$(dirname "$0")" && pwd); ROOT=$(cd "$HERE/../.." && pwd); BASE=d040e6d
...
PYTHONHASHSEED=0 $PY "$HERE/check_i4c.py" --final > /dev/null
mkdir -p "$HERE/diffs"
...
for c in s00_off s00_on s20_off s20_on s30_off s30_on s40_off s40_on; do
  ...
  for v in empty_only episode_only; do
