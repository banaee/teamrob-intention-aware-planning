#!/usr/bin/env python3
"""
compare.py — T5 regression comparison: the T7/T8 baselines (../t7_t8_meta_bugs/new, before the continue fix) vs new/ (HEAD).
...
Also reports, per condition, whether the full log is identical apart from [executor] lines, and lists every [executor] line that differs.
Writes comparison.md next to this file (summary.md is the hand-written record). Usage: python analysis/t5_continue/compare.py
"""
import re, os, sys
...
BASE_DIR = os.path.join(HERE, "..", "t7_t8_meta_bugs", "new")

def read(cond, which):
    with open(os.path.join(BASE_DIR if which == "baseline" else os.path.join(HERE, which), cond + ".log")) as f:
...
rows = ["# T5 sweep comparison — baseline (T7/T8 baselines, 4c5684e) vs new (HEAD, continue costs nothing)", "",
...
for cond in CONDS:
    ...
    nb = [ln for ln in base if not ln.startswith("[executor]")]; nn = [ln for ln in new if not ln.startswith("[executor]")]
    rows.append(f"- full log minus [executor] lines: {'byte-identical' if nb == nn else 'DIFFERS'} ({len(nb)} / {len(nn)} lines)")
    eb = [ln for ln in base if ln.startswith("[executor]")]; en = [ln for ln in new if ln.startswith("[executor]")]
    rows.append(f"- [executor] lines: {len(eb)} baseline / {len(en)} new; continue_plan in new: {sum('continue_plan' in ln for ln in en)}")
    ex = [f"- {b}\n+ {n}" for b, n in zip(eb, en) if b != n]
    if ex:
        rows.append("```"); rows.extend(ex); rows.append("```")
    rows.append("")
    sb, sn = set(db), set(dn)
