...
def final(base_dir, new_dir):
    ...
    from shared import likelihood_functions as LF
    from mesa_sim.action_decomposer import _get_beta    # the body's beta since T-A1
    base_dir, new_dir = Path(base_dir), Path(new_dir)
    inst = HERE / "logs_instrumented"; inst.mkdir(exist_ok=True)
    data = HERE / "data"; data.mkdir(exist_ok=True)
    env = dict(os.environ, PYTHONHASHSEED="0")
    S = [f"# Graded evidence — checks (generated by check.py; BETA={_get_beta(None)[0]}, UNKNOWN_LIKELIHOOD={LF.UNKNOWN_LIKELIHOOD}, θ={theta})\n"]
