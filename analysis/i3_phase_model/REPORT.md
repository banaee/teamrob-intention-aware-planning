...
| `sweep.sh`, `stages.sh` | `sweep.sh <repo_root> <out_dir>` runs the eight conditions through `run_mesa.py`. `stages.sh` regenerates every stage below from scratch (temporary worktrees of `5a8c480` with the two one-line patches, HEAD, the harness, the diffs). |
| `baseline/`, `stage1_zone/`, `stage2_held/`, `logs_instrumented/nopin/`, `new/` | The replay stages S0 → S1 (ZONE_BOOST = 1.0) → S2 (S1 + held-item rule returns ∅) → S3 (HEAD with `_terminal_complete` patched to False) → S4 (HEAD). **Not in git**: every `*.log` is gitignored (as I2's `logs_instrumented/` were); `stages.sh` brings them back in about two minutes. The variants `ungated` and `ownshelf` (`logs_instrumented/`) are likewise regenerated by `check_i3.py --variants`. The committed evidence is `summary.md`, the CSVs and `diffs/`. |
...
## 9. Hand-off for I4 (what the commit history does not say)

Reproduce anything here at `a7a4f8c` or later: `stages.sh` for the logs, `check_i3.py --variants`
for the harness and CSVs, `diff_ir.py` (I2's) for a tick-level attribution between two logs.

**(a) Where I4 attaches.** One kernel call site: `IntentionRecognizer._progress_likelihood(action,
origin, pos, world, memo)` in `shared/recognizer.py`, which reads
`likelihood_functions.PROGRESS_EVALUATORS[action.schema.progress_evaluator]` and calls it with
`(move_vec, origin, target_pos)`. `origin` is `_origin[key]`, set in `update()` at two places: when a
hypothesis is first observed (the priming observation, step −1) and when its expected action changes
(`if not self._same_action(previous, current)`). An odometer belongs in `update()` as one running
total per observed agent (`obs.spatial_context.position` deltas), and its reading at the origin
belongs in a dict beside `_origin`, reset at those same two places; the memo key in
`_progress_likelihood` — `("progress", evaluator, origin, target_pos)` — must then include the
origin reading, or two hypotheses that share an origin position but not an origin tick would share a
value they should not. Nothing in I3 reads an odometer (dropped on instruction).

**(b) TODO-53, the stuck-origin asymmetry, with the s40 numbers.** A hypothesis whose expected action
never completes keeps its t=0 origin (100, 550); under the cosine kernel its evidence is ONE chord
from there, replaced each tick, never charged for the path. `ac_activation` (switch at (400, 550)):
start → shelf_6 (950, −150) is 39° off the switch's bearing, L ≈ 3.5, so it is `most_likely` from 184
(coffee pinned) to 271 at 0.451 → 0.615 (off) / 0.544 → 0.693 (on), and at item_6's grasp (272) it is
0.479 / 0.525 against item_6's 0.294 / 0.322. item_6 lost because its origin WAS reset (to the table
at 115, via `deliver_with_return`'s flips) and its base carries the carry fold (×0.1–0.3). Same
mechanism: `coffee_break` `most_likely` at 0.646 / 0.687 at 115–117, before the coffee walk starts.
The human walked ≈ 2,000 cm from the start before reaching shelf_6; the switch is 300 cm from the
start. The excess-path likelihood reads exactly that; this is the first number it should move.

**(c) The bar scenario_40 now sets.** On `coffee_break` — the fixture's target — s40 has ZERO θ
crossings in both prior settings: max 0.551 (off) / 0.645 (on), at 118, falling to 0.51 / 0.56 by the
wait. Both I2 coffee crossings (142: item_3 losing ZONE_BOOST; 210: the foreseeable attractor, TODO-50)
were artefacts and are correctly gone. The two crossings that remain in s40 are on the human's
`deliver_item` carries (62 item_3, 325/326 item_6) — the positive control and segment 4, not the
foreseeable task. I4 therefore carries the whole burden of making this fixture produce a coffee
reveal: under the cosine kernel the coffee walk from the table is 4.0 for coffee against 3.2 for
item_5 (50° off), 2.9 for item_6 (58°) and 1.0 for `unknown`, and no phase mechanism changes that.
If the excess-path likelihood does not put `coffee_break` over θ during 118–186, there is no evidence
the design is right.

**(d) Two spec corrections I4 must not silently revert — design, not implementation.**
1. The completion channel is judged on the action the hypothesis expected BEFORE the event
   (`previous = self._expected[key]`), with the check against the current world. The literal order
   — derive the expected action against the post-event world, then ask whether its completion
   holds — fails by construction: after a grasp `holding(human, item)` already holds, so the walk
   has passed `pick_up` and the derived action is `move_to(table)`, whose completion does not hold;
   the correct hypothesis would receive LOW at its own grasp and HIGH could never occur. Check U4
   covers it (`check_i3.py`).
2. Bases are kept in ONE common scale (`_base`, every entry divided by the tick's total). Storing
   each hypothesis's base as its normalised evidence at its own advance tick (the per-tick spelling
   of step 3) carries that tick's normaliser into the ratio between hypotheses that advanced at
   different ticks: in s00 a rival advancing at tick 43, where item_3's chord is 4.0 and the tick
   total ≈ 3.6, would be charged ≈ ×3.6 relative to item_3 for nothing. The common scale makes the
   evidence exactly prior × Π(segment likelihoods) per hypothesis.
Also deliberate: the floor is on output only (no feedback into the evidence state), and the event
gate is the action's own vocabulary (TODO-56).

**(e) `unknown`'s flat likelihood is the common cause.** Every asymmetry the variants exposed traces
to `unknown` paying nothing: it is `most_likely` after every release in the shipped code (0.38–0.71);
it crosses θ when a pin shrinks the live set (s20_off 136, 0.804, TODO-54); under the ungated channel
it takes ×10 against every live task at every grasp and release and ends at 0.96–0.99 (TODO-56); and
it is the opponent the next task cannot beat before its grasp after a carry (TODO-55). If I4 gives
`unknown` a likelihood of its own, TODO-54/55/56 should be re-read together with it.

**(f) TODO-55 is DEFERRED by decision.** Its severity (≈ ×0.1 per carry on every rival) is a
LOW_LIKELIHOOD-cliff number; I4 replaces the cliff with a gradient, so its five readings are to be
re-measured after the I4 sweep, not chosen in passing.
